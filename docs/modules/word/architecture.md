...
Source root: `src/md_generator/word` (9 Python modules detected).
...
```mermaid
flowchart TB
    CLI[CLI_md-word] --> Core[Core_engine]
...
- **Outputs:** Markdown with optional embedded images
- **Optional extras:** `word` from `pyproject.toml`
