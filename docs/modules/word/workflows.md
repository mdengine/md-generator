...
1. Install `mdengine[word]`.
2. Run `md-word --help` to list flags.
3. Provide input (DOCX documents) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[word,api]`.
2. Start uvicorn on `md_generator.word` API module (see Deployment).
