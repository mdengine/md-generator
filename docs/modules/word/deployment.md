...
Install extras in container image; invoke `md-word` as Job/Cron.

## Docker API

If `word-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /word-to-md` behind gateway (`deploy/docker-compose.yml`).
