...
Install extras in container image; invoke `md-image` as Job/Cron.

## Docker API

If `image-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /image-to-md` behind gateway (`deploy/docker-compose.yml`).
