...
1. Install `mdengine[skill-openai]`.
2. Run `mdengine ai assist --help` to list flags.
3. Provide input (Skill bundles and prompts) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[skill-openai,api]`.
2. Start uvicorn on `md_generator.tools.assistant` API module (see Deployment).
