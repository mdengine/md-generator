...
Install extras in container image; invoke `md-audio` as Job/Cron.

## Docker API

If `audio-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /audio-to-md` behind gateway (`deploy/docker-compose.yml`).
