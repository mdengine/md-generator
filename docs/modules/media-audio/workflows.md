...
1. Install `mdengine[audio]`.
2. Run `md-audio --help` to list flags.
3. Provide input (Audio files) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[audio,api]`.
2. Start uvicorn on `md_generator.media.audio` API module (see Deployment).
