...
1. Install `mdengine[openapi]`.
2. Run `md-openapi --help` to list flags.
3. Provide input (OpenAPI 3.x or Swagger 2.0) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[openapi,api]`.
2. Start uvicorn on `md_generator.openapi` API module (see Deployment).
