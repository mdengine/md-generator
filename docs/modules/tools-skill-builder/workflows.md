...
1. Install `mdengine[(base]`.
2. Run `mdengine skill build --help` to list flags.
3. Provide input (Project metadata) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[(base,api]`.
2. Start uvicorn on `md_generator.tools.skill_builder` API module (see Deployment).
