...
Source root: `src/md_generator/tools/skill_builder` (6 Python modules detected).

Subpackages and areas: `(package root)`.

## Component diagram

```mermaid
flowchart TB
    CLI[CLI_mdengine skill build] --> Core[Core_engine]
...
- **Outputs:** Structured skills under ai/
- **Optional extras:** `(base package)` from `pyproject.toml`
