...
Install extras in container image; invoke `mdengine skill build` as Job/Cron.

## Docker API

If `tool-skill-builder/Dockerfile.api` exists, build and run uvicorn with `--root-path /tool-skill-builder` behind gateway (`deploy/docker-compose.yml`).
