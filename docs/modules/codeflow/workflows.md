...
1. Install `mdengine[codeflow]`.
2. Run `md-codeflow --help` to list flags.
3. Provide input (Source repositories) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[codeflow,api]`.
2. Start uvicorn on `md_generator.codeflow` API module (see Deployment).
