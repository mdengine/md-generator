...
Source root: `src/md_generator/media/video` (10 Python modules detected).
...
```mermaid
flowchart TB
    CLI[CLI_md-video] --> Core[Core_engine]
...
- **Inputs:** Video files
- **Outputs:** Transcript Markdown with video metadata
- **Optional extras:** `video` from `pyproject.toml`
