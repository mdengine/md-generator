...
1. Install `mdengine[video]`.
2. Run `md-video --help` to list flags.
3. Provide input (Video files) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[video,api]`.
2. Start uvicorn on `md_generator.media.video` API module (see Deployment).
