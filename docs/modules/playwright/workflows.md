...
1. Install `mdengine[playwright]`.
2. Run `md-playwright --help` to list flags.
3. Provide input (Rendered web pages and SPAs) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[playwright,api]`.
2. Start uvicorn on `md_generator.playwright` API module (see Deployment).
