...
Source root: `src/md_generator/pdf` (10 Python modules detected).
...
```mermaid
flowchart TB
    CLI[CLI_md-pdf] --> Core[Core_engine]
...
- **Outputs:** Markdown with optional artifact layout and extracted images
- **Optional extras:** `pdf` from `pyproject.toml`
