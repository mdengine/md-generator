...
1. Install `mdengine[pdf]`.
2. Run `md-pdf --help` to list flags.
3. Provide input (PDF documents) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[pdf,api]`.
2. Start uvicorn on `md_generator.pdf` API module (see Deployment).
