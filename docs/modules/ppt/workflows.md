...
1. Install `mdengine[ppt]`.
2. Run `md-ppt --help` to list flags.
3. Provide input (PPTX slide decks) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[ppt,api]`.
2. Start uvicorn on `md_generator.ppt` API module (see Deployment).
