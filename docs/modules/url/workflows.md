...
1. Install `mdengine[url]`.
2. Run `md-url --help` to list flags.
3. Provide input (HTTP(S) pages) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[url,api]`.
2. Start uvicorn on `md_generator.url` API module (see Deployment).
