...
Source root: `src/md_generator/graph` (28 Python modules detected).
...
```mermaid
flowchart TB
    CLI[CLI_md-graph] --> Core[Core_engine]
...
- **Inputs:** Neo4j or NetworkX GraphML/GML
- **Outputs:** Node/relationship Markdown, Mermaid, Graphviz
- **Optional extras:** `graph` from `pyproject.toml`
