...
Install extras in container image; invoke `md-graph` as Job/Cron.

## Docker API

If `graph-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /graph-to-md` behind gateway (`deploy/docker-compose.yml`).
