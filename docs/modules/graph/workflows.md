...
1. Install `mdengine[graph]`.
2. Run `md-graph --help` to list flags.
3. Provide input (Neo4j or NetworkX GraphML/GML) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[graph,api]`.
2. Start uvicorn on `md_generator.graph` API module (see Deployment).
