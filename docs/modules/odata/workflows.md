# OData Metadata Workflows

## CLI workflow

1. Install `mdengine[odata]`.
2. Run `md-odata --help` to list flags.
3. Provide input (OData CSDL metadata (XML/JSON), folders, ZIP archives, or $metadata URLs) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[odata,api]`.
2. Start uvicorn on `md_generator.odata` API module (see Deployment).
