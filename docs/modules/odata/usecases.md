# OData Metadata Use Cases

### Local developer conversion

| Aspect | Detail |
|--------|--------|
| Objective | Convert a single OData CSDL metadata (XML/JSON), folders, ZIP archives, or $metadata URLs during feature work. |
| Input | Sample OData CSDL metadata (XML/JSON), folders, ZIP archives, or $metadata URLs |
| Output | Entity catalog Markdown, optional graph and semantic chunks |
| Configuration | `pip install -e ".[odata]"` |
...
| Input | Entity catalog Markdown, optional graph and semantic chunks |
