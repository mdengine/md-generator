...
1. Install `mdengine[text]`.
2. Run `md-text --help` to list flags.
3. Provide input (TXT, JSON, XML) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[text,api]`.
2. Start uvicorn on `md_generator.text` API module (see Deployment).
