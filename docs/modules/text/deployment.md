...
Install extras in container image; invoke `md-text` as Job/Cron.

## Docker API

If `txt-json-xml-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /txt-json-xml-to-md` behind gateway (`deploy/docker-compose.yml`).
