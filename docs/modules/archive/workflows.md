...
1. Install `mdengine[archive]`.
2. Run `md-zip --help` to list flags.
3. Provide input (ZIP archives) and output path.
4. Inspect generated Markdown and sidecar assets.

## API workflow

1. Install `mdengine[archive,api]`.
2. Start uvicorn on `md_generator.archive` API module (see Deployment).
