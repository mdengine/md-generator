# ZIP Archive Architecture

## Internal layout

Source root: `src/md_generator/archive` (13 Python modules detected).
...
```mermaid
flowchart TB
    CLI[CLI_md-zip] --> Core[Core_engine]
...
- **Inputs:** ZIP archives
- **Outputs:** Directory-oriented Markdown bundle
- **Optional extras:** `archive plus nested format extras` from `pyproject.toml`
