...
Install extras in container image; invoke `md-zip` as Job/Cron.

## Docker API

If `zip-to-md/Dockerfile.api` exists, build and run uvicorn with `--root-path /zip-to-md` behind gateway (`deploy/docker-compose.yml`).
