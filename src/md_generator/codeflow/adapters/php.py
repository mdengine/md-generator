...
class PHPAdapter(LanguageAdapter):
    ...
    def discover(self, workspace_root: Path) -> list[Path]:
        return sorted(list(workspace_root.rglob("*.php")) + list(workspace_root.rglob("composer.json")))
                        ...
                        for name, ver in data[k].items():
                            deps.append(
                                RawDependency(
                                    name=name,
                                    version=ver,
                                    scope="compile" if k == "require" else "dev",
                                )
                            )
            except Exception:
                pass
    ...
    def extract_config_usage(self, file_path: Path, backend: str) -> list[ConfigUsage]:
        ...
        try:
            ...
            matches = re.finditer(r"getenv\([\'\"]([a-zA-Z0-9_]+)[\'\"]\)", content)
            for m in matches:
                key = m.group(1)
