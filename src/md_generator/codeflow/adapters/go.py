...
class GoAdapter(LanguageAdapter):
    ...
    def discover(self, workspace_root: Path) -> list[Path]:
        return sorted(list(workspace_root.rglob("*.go")) + list(workspace_root.rglob("go.mod")))
    ...
    def extract_dependencies(self, file_path: Path, backend: str) -> list[RawDependency]:
        ...
        if file_path.name == "go.mod":
            try:
                ...
                # Look for require lines
                matches = re.finditer(r"\brequire\s+([a-zA-Z0-9_\-\./]+)\s+([a-zA-Z0-9_\-\.\+]+)", content)
                for m in matches:
                    deps.append(
                        RawDependency(
                            name=m.group(1),
    ...
    def extract_config_usage(self, file_path: Path, backend: str) -> list[ConfigUsage]:
        ...
        try:
            content = file_path.read_text(encoding="utf-8", errors="replace")
            matches = re.finditer(r"os\.Getenv\([\'\"]([a-zA-Z0-9_]+)[\'\"]\)", content)
            for m in matches:
                key = m.group(1)
